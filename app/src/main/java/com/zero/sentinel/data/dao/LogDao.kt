...
interface LogDao {
    ...
    @Query("SELECT * FROM logs ORDER BY timestamp ASC") suspend fun getAllLogs(): List<LogEntry>

    @Query("DELETE FROM logs WHERE id IN (:ids)") suspend fun deleteByIds(ids: List<Long>)
